...
const Web3ContextProvider = ({children}) => {
  const [web3Onboard, setWeb3Onboard] =
    useState(null);
  ...
  const [daiContract, setDaiContract] =
    useState(null);
  const [alphaContract, setAlphaContract] =
    useState(null);
  const [
    gammaPacksContract,
    setGammaPacksContract
  ] = useState(null);
  const [
    gammaCardsContract,
  ...
  const initWeb3Onboard =
    useCallback(async () => {
      ...
      const walletConnect = walletConnectModule(
        wcV2InitOptions || wcV1InitOptions
      );
  ...
  */
  ...
  function connectContracts(_signer) {
    try {
      ...
      const alphaContractInstance =
        new ethers.Contract(
          CONTRACTS.alphaAddress,
          alphaAbi.abi,
          _signer
        );
      const gammaPacksContractInstance =
        new ethers.Contract(
          CONTRACTS.gammaPackAddress,
          gammaPacksAbi.abi,
          _signer
        );
      const gammaCardsContractInstance =
        new ethers.Contract(
          CONTRACTS.gammaCardsAddress,
          gammaCardsAbi.abi,
          _signer
        );
      const gammaOffersContractInstance =
        new ethers.Contract(
