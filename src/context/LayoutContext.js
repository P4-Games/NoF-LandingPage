...
function LayoutProvider({children}) {
  ...
  const [
    footerButtonsShowDefaults,
    setShowDefaultButtons
  ] = useState(true);
  const [
    footerButtonsFunctions,
    setFooterButtonsFunctions
  ] = useState([null, null, null, null]);
  const [
    footerButtonsToShow,
    setFooterButtonsToShow
  ] = useState([true, true, true, true]);
  const [
    footerButtonsClasses,
    setFooterButtonsClasses
  ] = useState(null, null, null, null);
  ...
  const updateFooterButtonsFunctions =
    useCallback((index, newFunction) => {
      setFooterButtonsFunctions(
        (prevFunctions) => {
  ...
  const updateFooterButtonsClasses = useCallback(
    (cls1, cls2, cls3, cls4) => {
      setFooterButtonsClasses(
        cls1,
        cls2,
  ...
  const updateFooterButtonsToShow = useCallback(
    (btn1, btn2, btn3, btn4) => {
      setFooterButtonsToShow(
        btn1,
        btn2,
  ...
  const ToggleShowDefaultButtons = useCallback(
    (toggle) => {
  ...
  const turnNextPage = useCallback(() => {
    ...
    bookRef.current
      .pageFlip()
      .flipNext("bootom", true);
  }, []);

  const getCurrentPage = useCallback(() => {
    if (!bookRef || !bookRef.current) return 0;
    const currentPage = bookRef.current
      .pageFlip()
      .getCurrentPageIndex();
    return currentPage;
  }, []);
    ...
    <LayoutContext.Provider
      value={{
        ...
        updateButtonFunctions:
          updateFooterButtonsFunctions,
        updateShowButtons:
          updateFooterButtonsToShow,
