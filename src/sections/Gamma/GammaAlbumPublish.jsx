...
const GammaAlbumPublish = (props) => {
  ...
  const handleConfirmClick =
    useCallback(async () => {
      if (selectedCards.length === 0) {
        emitInfo(
          t("publish_offer_no_cards_selected")
        );
        return;
      }
  ...
  const handleCardClick = async (
    ...
    if (
      parseInt(selectedCard) ===
      parseInt(cardNumberOffered)
    ) {
      emitWarning(
        t("publish_offer_error_own_card_number")
      );
    } else {
      if (selectedCards.includes(selectedCard)) {
        setSelectedCards((pevSelectedCards) =>
          pevSelectedCards.filter(
    ...
    <div
      ...
      {paginationObj.user[item]?.quantity > 1 && (
        <div className='quantity'>
          X:{paginationObj.user[item]?.quantity}
        </div>
      )}
      {
        <div className='number'>
          {paginationObj.user[item]?.name || "0"}
        </div>
      }
    </div>
  );
