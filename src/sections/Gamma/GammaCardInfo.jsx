...
const GammaCardInfo = (props) => {
  ...
  const [userHasCard, setUserHasCard] =
    useState(false);
  const [cardPublish, setCardPublish] =
    useState(false);

  const verifyUserHasCard = async () => {
    try {
      startLoading();
      const result = await hasCard(
        gammaCardsContract,
        walletAddress,
        userCard.name
      );
  ...
  const handleTransferClick = async () => {
    try {
      ...
      if (result.isConfirmed) {
        startLoading();
        const transaction =
          await gammaCardsContract.transferCard(
            result.value,
            userCard.name
          );
  ...
  const handleMintClick = async () => {
    try {
      startLoading();
      const transaction =
        await gammaCardsContract.mintCard(
          userCard.name
        );
      await transaction.wait();
      handleFinishInfoCard(true);
  ...
  const handlePublishClick = async () => {
    ...
    const canUserPublishResult =
      await canUserPublishOffer(
        gammaOffersContract,
        walletAddress
      );
    if (!canUserPublishResult) {
      ...
      const canAnyUserPublishResult =
        await canAnyUserPublishOffer(
          gammaOffersContract
        );
  ...
  const Page1 = () => (
    <div className='cardinfo'>
      <div className='cardinfoimg'>
        ...
        />
        {userCard.offered && (
          <MdOutlineLocalOffer className='cardinfoimg-offered' />
        )}
      </div>
      <h3>#{userCard.name}</h3>
