...
const GammaCardOffers = (props) => {
  ...
  const existInSomeOfferWantedCards = (
    cardNumber
  ) => {
    ...
    for (
      ...
      for (
        let index2 = 0;
        index2 < offer.wantedCards.length;
        index2++
      ) {
        const wantedCard =
          offer.wantedCards[index2];
  ...
  function findFirstOfferByCardNumber(
    cardNumber
  ) {
    // Busca el elemento que contiene el cardNumber en wantedCards
    // deveulve el primero
    const offers = offerData.filter((item) =>
      item.wantedCards.includes(cardNumber)
    );
  ...
  const handleExchangeClick = async (item) => {
    try {
      ...
      const offer =
        findFirstOfferByCardNumber(item);
