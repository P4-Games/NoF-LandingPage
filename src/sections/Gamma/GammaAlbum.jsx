...
import {
  getOffersByCardNumber /*, getOffers*/
...
const GammaAlbum = (props) => {
  ...
  const {gammaOffersContract, walletAddress} =
    useWeb3Context();
  const [cardInfo, setCardInfo] = useState(false);
  const [cardOffers, setCardOffers] =
    useState(false);
  const [imageNumber, setImageNumber] =
    useState(0);
  const [offersObj, setOffersObj] =
    useState(null);
  ...
  */
  ...
  const handleOpenCardOffers = async () => {
    if (!offersObj || offersObj.length === 0) {
      const myOffer =
        paginationObj.user[imageNumber]?.offered;
      if (myOffer)
        emitInfo(t("offer_only_own_offer"), 5500);
      else
        emitInfo(
          t("offer_card_number_empty"),
          5500
        );
      return;
    }
  ...
  const updateCardOffers = async (cardNumber) => {
    ...
    if (offers && offers.length > 0) {
      ...
      const filterMyoffes = offers.filter(
        (item) =>
          item.offerWallet !== walletAddress
      );
  ...
  */
        ...
        {page &&
          page.map((item, index) => (
            <div
              ...
              className='grid-item'>
              <CustomImage
                src={`${storageUrlGamma}/T1/${item}.png`}
                alt='img'
              />
              {paginationObj.user[item]
                ?.stamped && <FcCheckmark />}
              {paginationObj.user[item]
                ?.offered && (
  ...
  const PageContentAlbum = ({
    page,
    pageNumber
  }) => {
    let divWrapperClassName =
  ...
  const getuserCardObject = (imageNumber) => {
    const data = paginationObj
      ? Object.values(paginationObj.user).find(
          (entry) =>
            entry.name === imageNumber.toString()
        )
      : {};
    return data;
  };
