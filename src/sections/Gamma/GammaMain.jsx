...
const GammaMain = () => {
  ...
  const [
    openPackCardsNumbers,
    setOpenPackCardsNumbers
  ] = useState([]);
  const [numberOfPacks, setNumberOfPacks] =
    useState(0);
  const [openPackage, setOpenPackage] =
    useState(false);
  const [inventory, setInventory] =
    useState(true);
  const [packIsOpen, setPackIsOpen] =
    useState(false);
  const {
    walletAddress,
    daiContract,
  ...
  const [paginationObj, setPaginationObj] =
    useState({});
  const [cardsQtty, setCardsQtty] = useState(0);
  const [showRules, setShowRules] =
    useState(false);
  const [cardInfoOpened, setCardInfoOpened] =
    useState(false);
  ...
  const checkNumberOfPacks = async () => {
    try {
      const result = await checkPacksByUser(
        walletAddress,
        gammaPacksContract
      );
  ...
  const updateUserData = async () => {
    const userCards = await getCardsByUser(
      gammaCardsContract,
      walletAddress
    );
  ...
  const handleFinishAlbum =
    useCallback(async () => {
      try {
        ...
        startLoading();
        const result = await finishAlbum(
          gammaCardsContract,
          daiContract,
          walletAddress
        );
  ...
  const handleTransferPack =
    useCallback(async () => {
      try {
        if (numberOfPacks === 0) {
          emitInfo(
            t("no_paquetes_para_transferir", 2000)
          );
          return;
        }
        ...
        if (result.isConfirmed) {
          startLoading();
          const packs = await checkPacksByUser(
            walletAddress,
            gammaPacksContract
          );
          const packNumber =
          ...
          const transaction =
            await gammaPacksContract.transferPack(
              result.value,
              packNumber
            );
  ...
  const handleOpenPack = useCallback(async () => {
    try {
      ...
      setPackIsOpen(true);
      const packNumber = ethers.BigNumber.from(
        packs[0]
      ).toNumber();
  ...
  const buyPacksContract = async (
    ...
    */
    ...
    try {
      ...
      const call =
        await gammaPacksContract.buyPacks(
          numberOfPacks,
          {gasLimit: 6000000}
        );
      await call.wait();
          ...
          <div className='gammapack'>
            ...
            cardInfoOpened ? (
              <>
                <div
                  className={
                    "gammapack__content__disabled"
                  }>
                  <h1
                    className={
                      "pack_number_disabled"
                    }>
                    {numberOfPacks}
                  </h1>
                </div>
              ...
              cardInfoOpened ? (
                <>
                  {cardInfoOpened ? (
                    <div
                      className={
                        "gammapack__actions__buyPack_disabled"
                      }>
                      <Image
                        src={
                          "/images/gamma/buyPackOff.png"
                        }
                        alt='buy pack'
                        height='40'
                        width='40'
                    ...
                    <div
                      ...
                      className={
                        "gammapack__actions__buyPack"
                      }>
                      <Image
                        src={
                          "/images/gamma/buyPackOn.png"
                        }
                        alt='buy pack'
                        height='40'
                        width='40'
                  ...
                  <div className='gammapack__actions__transferPack_disabled'>
                    <Image
                      src={
                        "/images/gamma/transferPackOff.png"
                      }
                      alt='open pack'
                      height='40'
                      width='40'
                ...
                <>
                  ...
                  <div
                    ...
                    <Image
                      src={
                        "/images/gamma/transferPackOn.png"
                      }
                      alt='open pack'
                      height='40'
                      width='40'
