...
const AlphaAlbums = ({
  ...
  const noAlbumMessage = t(
    "juega_para_completar"
  );
  const [seasonNameAlbum, setSeasonNameAlbums] =
    useState("");
  ...
  function handleRedirectAlbum(album) {
    if (album[0].completion === 5) {
      ...
      window.open(
        `${openSeaUrlAlpha}/${album[0].tokenId}`,
        "_blank"
      );
    } else {
  ...
  return (
    <div className='alpha_full_albums_container alpha_display_none'>
      <div>
        ...
        {!albums && (
          <div
            className='alpha_no_album_message'
            onClick={() => clickFromAlbums()}>
            {noAlbumMessage}
          </div>
        )}
      </div>
