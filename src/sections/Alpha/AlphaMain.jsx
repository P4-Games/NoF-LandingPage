...
const AlphaMain = () => {
  ...
  const [albumImage, setAlbumImage] =
    useState(null);
  const [albumCollection, setAlbumCollection] =
    useState(null);
  const [albumCompletion, setAlbumCompletion] =
    useState(null);
  const [isCollection, setIsCollection] =
    useState(false);
  ...
  const [seasonNames, setSeasonNames] =
    useState(null);
  const [seasonName, setSeasonName] =
    useState("");
  const [packPrices, setPackPrices] =
    useState(null);
  const [packPrice, setPackPrice] = useState("");
  const [receiverAccount, setReceiverAccount] =
    useState("");
  const [cardToTransfer, setCardToTransfer] =
    useState(null);
  const [winnerPosition, setWinnerPosition] =
    useState(0);
  const [transferError, setTransferError] =
    useState("");
  const [, setDisableTransfer] = useState(false);
  const [seasonFolder, setSeasonFolder] =
    useState(null);
  ...
  function alphaMidButton() {
    ...
    if (game && game.length > 0) {
      albums[0].classList.toggle(
        "alpha_display_none"
      );
      game[0].classList.toggle(
        "alpha_display_none"
      );
    }
  }
  ...
  const fetchAlbums = async () => {
    try {
      ...
      for (
        let i = 0;
        i < seasonNames.length;
        i++
      ) {
        const album =
          await alphaContract.getCardsByUserBySeason(
            walletAddress,
            seasonNames[i]
          );
        for (let j = 0; j < album.length; j++) {
          if (album[j].class == 0) {
            const folder = await getSeasonFolder(
              album[j].season
            );
  ...
  const fetchSeasonData = async () => {
    try {
      if (!walletAddress || !alphaContract)
        return;
      startLoading();
      let seasonData =
        await alphaContract.getSeasonData();
      ...
      if (seasonData) {
        ...
        for (
          let i = 0;
          i < seasonData[0].length;
          i++
        ) {
          const season =
            await alphaContract.getSeasonAlbums(
              seasonData[0][i]
            );
        ...
        const finishedSeasons = Object.entries(
          seasonWinnersCount
        )
        ...
        const activeSeasons =
          seasonData[0].filter(
            (season) =>
              !finishedSeasons.includes(season)
          );
      ...
      console.error(ex);
      emitError(
        t("alpha_fetch_season_data_error")
      );
    }
  };
    ...
    swiper = new Swiper(".swiper-container", {
      ...
      on: {
        init: (res) => {
          ...
          if (cards.length > 0) {
            if (
              cards[res.activeIndex].collection ==
              albumCollection
            ) {
    ...
    if (seasonName) {
      ...
      if (seasonName.length > 16) {
        ...
        seasonNameElem.innerText =
          seasonNameElem.innerText.substring(
            0,
            16
          ) + "...";
      }
  ...
  const getSeasonFolder = async (seasonName) => {
    try {
      const response =
        await alphaContract.seasons(seasonName);
  ...
  const getWinners = async () => {
    try {
      const winners =
        await alphaContract.getWinners(
          seasonName
        );
      return winners;
    } catch (ex) {
  ...
  const getAlbumData = async (tokenId) => {
    const albumData =
      await alphaContract.cards(tokenId);
    return albumData;
  };
          ...
          if (pack.length) {
            ...
            getSeasonFolder(seasonName)
              .then((data) => {
                ...
                const baseUrl = `${storageUrlAlpha}/${
                  seasonFolder || "T1"
                }`;
                if (completion < 5) {
                  setAlbumImage(
                    `${baseUrl}/${
                      albumData[0].number + ".png"
                    }`
                  );
                } else {
                  setAlbumImage(
  ...
  const pasteCard = (cardIndex) => {
    try {
      ...
      const pegarCarta = async (cardIndex) => {
        ...
        const paste =
          await alphaContract.pasteCards(
            tokenId,
            albumTokenId,
            {
              gasLimit: 2500000
  ...
  async function transferToken() {
    try {
      ...
      if (
        e.reason.includes(
          "Receiver is not playing this season"
        )
      ) {
        setTransferError(
          t("usuario_no_en_temporada")
        );
      }
    }
  }
            ...
            <div
                    ...
                    <>
                      <div className='alpha_season'>
                        <img
                          alt='marco'
                          src={
                            "/images/common/marco.png"
                          }
                        />
                        <span className='alpha_season_name'>
                          {seasonName}
                        </span>
                        <select
                          value={seasonName}
              ...
              {pack && pack.length && showMain ? (
                <div className='alpha_container'>
                  <div
                    className='alpha_album_container'
                    onClick={() =>
                      alphaMidButton()
                    }>
                    <CustomImage
                  ...
                  {cards.length > 0 ? (
                    <div className='alpha_transfer_modal alpha_display_none'>
                      ...
                      <span className='alpha_transfer_error'>
                        {transferError}
                      </span>
                    </div>
                  ) : null}
