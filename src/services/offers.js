...
export const createOffer = async (
  ...
  for (const wantedCard of wantedCardNumbers) {
    if (wantedCard === cardNumber) {
      throw new Error(
        "publish_offer_error_own_card_number"
      );
    }
  }
  const trx = await offersContract.createOffer(
    uuidv4(),
    cardNumber,
    wantedCardNumbers
  );
...
export const removeOfferByCardNumber = async (
  offersContract,
  cardNumber
) => {
  try {
    const trx =
      await offersContract.removeOfferByCardNumber(
        cardNumber
      );
    await trx.wait();
  } catch (e) {
...
export const canUserPublishOffer = async (
  offersContract,
  walletAddress
) => {
  try {
    const result =
      await offersContract.canUserPublishOffer(
        walletAddress
      );
    return result;
  } catch (e) {
...
export const canAnyUserPublishOffer = async (
  offersContract
) => {
  try {
    const result =
      await offersContract.canAnyUserPublishOffer();
    return result;
  } catch (e) {
    console.error({e});
    throw e;
...
export const getOffersByCardNumber = async (
  offersContract,
  cardNumber
) => {
  try {
    ...
    // [0][4] timeStamp
    const offers =
      await offersContract.getOffersByCardNumber(
        cardNumber
      );
    if (!offers) return [];
    ...
    const filteredResult = offerObject.filter(
      (item) => item.offerId !== 0
    );
