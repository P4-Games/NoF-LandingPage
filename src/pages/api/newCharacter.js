...
const getRandomCharacterID = async (
  ...
  if (!server) {
    throw new Error(
      `Server not found for channelId: ${channelId}`
    );
  }
...
const findCharacterByID = async (
  ...
  const character =
    await charactersCollection.findOne({
      image: characterImage
    });

  if (!character) {
    throw new Error(
      `Character not found or missing image for characterID: ${characterID}`
    );
  }
...
const addCharacterToInventory = async (
  db,
  userID,
  characterID,
  characterImage
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const user = await findUserByDiscordID(
      db,
      discordID
    );
    ...
    const characterID =
      await getRandomCharacterID(db, channelID);
    ...
    if (characterID === null) {
      return res
        ...
        .json({
          message:
            "The character has already been collected."
        });
    }

    const character = user.characters.find(
      (c) =>
        c.id.toString() === characterID.toString()
    );
    ...
    if (character) {
      return res
        ...
        .json({
          message:
            "You already have this character in your inventory."
        });
    }

    const characterObj = await findCharacterByID(
      db,
      characterID
    );
