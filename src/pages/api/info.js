...
const getUserInfo = async (db, discordID) => {
  ...
  if (!user) {
    throw new Error(
      `User not found for discordID: ${discordID}`
    );
  }
  ...
  const inventoryCompletion = Math.floor(
    (charactersCount / 132) * 100
  );
  ...
  const userRanking = await usersCollection
    .find()
    .toArray();
  const sortedRanking =
    calculateRank(userRanking);
...
const calculateRank = (users) =>
  users.sort((a, b) => {
    ...
    const aMedalRank = calculateMedalRank(
      a.medals
    );
    const bMedalRank = calculateMedalRank(
      b.medals
    );
...
const calculateMedalRank = (medals) => {
  ...
  const silverMedals = countMedals(
    medals,
    "silver"
  );
  const bronzeMedals = countMedals(
    medals,
    "bronze"
  );
...
const countMedals = (medals, medalType) =>
  medals.filter((medal) => medal === medalType)
    .length;
