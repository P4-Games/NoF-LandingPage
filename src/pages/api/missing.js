...
export default async function handler(req, res) {
  ...
  if (!discordID) {
    return res
      ...
      .json({
        error:
          "Discord ID is missing from query parameters."
      });
  }
  ...
  try {
    ...
    const charactersNotInInventory =
      characters.filter(
        (character) =>
    ...
    const characterImagePaths =
      charactersNotInInventory.map((c) =>
        join(
          process.cwd(),
          "scripts",
          "characters",
    ...
    const imagePromises = characterImagePaths.map(
      async (imagePath) =>
    ...
    const maxImagesPerRow = getNumberOfColumns(
      images.length
    );
    ...
    const collageWidth =
      maxImagesPerRow * imageWidth;
    const collageHeight =
      maxImagesPerColumn * imageHeight;

    // Crear un lienzo para el collage
    const collage = new Jimp(
      collageWidth,
      collageHeight
    );
    ...
    const collageBuffer =
      await collage.getBufferAsync(Jimp.MIME_PNG);
