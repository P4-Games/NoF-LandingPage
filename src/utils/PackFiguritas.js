  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const CYCLE_DURATION =
        STAGGER * BOXES.length;
      const START_TIME =
        CYCLE_DURATION + DURATION * 0.5 + OFFSET;
      ...
      const POSITION_WRAP = gsap.utils.wrap(
        0,
        LOOP_HEAD.duration()
      );
      ...
      const SCRUB = gsap.to(PLAYHEAD, {
        ...
        onUpdate: () => {
          LOOP_HEAD.totalTime(
            POSITION_WRAP(PLAYHEAD.position)
          );
        },
        paused: true,
        duration: 0.25,
        ...
        .addEventListener("click", (e) => {
          ...
          if (BOX) {
            ...
            const CURRENT = gsap.utils.wrap(
              ...
              BOXES.length,
              Math.floor(
                BOXES.length * SCRUB.vars.position
              )
            );
